"""Corrected repeat of scripts/measure_load_int8.py — SAME quantization
strategy and architecture (torch.ao.quantization.quantize_dynamic on
GCTStream.aggregator's nn.Linear layers, dtype=torch.qint8), with ONE
change: explicit `del` of the temporary FP32 aggregator + intermediate
objects, followed by `gc.collect()`, immediately after conversion — plus
finer-grained per-stage instrumentation to separate two distinct
phenomena that the previous run conflated:

  (a) the PEAK reached transiently *during* quantize_dynamic() itself —
      quantize_dynamic(inplace=False, the default, unchanged here) first
      deep-copies the input module before converting the copy's Linear
      layers one at a time. That deep-copy of the whole (fp32) aggregator
      is a plausible structural cause of a large transient peak — and a
      del+gc.collect() called AFTER the function returns cannot reduce a
      peak that already happened DURING the call. This experiment does
      not change `inplace` (that would be a different strategy, out of
      scope here per instruction) — it only measures whether this
      hypothesis holds, using the OS-tracked PeakWorkingSetSize /
      PeakPagefileUsage counters (continuous, not sampling-dependent —
      the same instrumentation used throughout this investigation).

  (b) the STEADY-STATE memory left over once quantize_dynamic() has
      returned and model.aggregator has been reassigned — THIS is what
      explicit del(original fp32 aggregator, the returned quantized copy
      var) + gc.collect() should fix, if the earlier high number
      (12456.4MB) was indeed unreleased temporaries rather than an
      inherent property of the quantized model itself.

7 explicit stages (exact order requested):
  1. modelo FP32 cargado
  2. inicio de cuantización (snapshot immediately before calling quantize_dynamic)
  3. pico durante cuantización (snapshot immediately after the call returns —
     its peak_ws/peak_pagefile fields reflect the true max reached inside
     the call, per the OS counters)
  4. cuantización completada (snapshot after model.aggregator is reassigned
     to the quantized result — still before cleanup)
  5. del de referencias FP32 + gc.collect()
  6. estado estable INT8 (after eval() + functional checks)
  7. inferencia (before/after)

Does NOT touch demo.py. Does NOT change the quantization strategy
(inplace stays False, same as before). Does NOT use torchao. Does NOT
test INT4. Single repeat, per instruction.
"""
import argparse
import ctypes
...
def stage(label):
    ...
    print(
        f"[STAGE] t={time.time() - _T0:7.1f}s {label:52s} "
...
def main():
    ...
    # ============================== PHASE A: FP32 baseline (unchanged) ==============================
    ...
    # ============================== PHASE B: INT8 dynamic quantization, CORRECTED cleanup ==============================
    print("\n########## PHASE B: INT8 dynamic quantization — corrected (explicit del + gc.collect) ##########", flush=True)
    t0 = time.time()
    model_int8 = build_and_load(args, device)
    load_time_int8_fp32_stage = time.time() - t0
    print(f"LOAD_TIME_FP32_STAGE_S={load_time_int8_fp32_stage:.2f}", flush=True)

    # ---- STAGE 1: modelo FP32 cargado ----
    stage("STAGE 1: fp32 model loaded, ready to quantize")

    n_linear = count_linear_layers(model_int8.aggregator)
    print(f"  nn.Linear layers found in aggregator: {n_linear}", flush=True)

    # ---- STAGE 2: inicio de cuantizacion ----
    stage("STAGE 2: about to call quantize_dynamic()")

    # Hold an EXPLICIT local reference to the fp32 aggregator being replaced,
    # so it can be del'd explicitly below (not just relying on implicit
    # refcounting via reassignment).
    fp32_aggregator_ref = model_int8.aggregator

    t_quant0 = time.time()
    quantized_aggregator = quantize_dynamic(fp32_aggregator_ref, {nn.Linear}, dtype=torch.qint8)
    quant_time = time.time() - t_quant0

    # ---- STAGE 3: pico durante cuantizacion ----
    # peak_ws/peak_pagefile below are OS-tracked continuously — they reflect
    # the true maximum reached DURING the quantize_dynamic() call above,
    # regardless of what the instantaneous ws/private read right now.
    stage("STAGE 3: quantize_dynamic() returned (peak_* fields = true max during the call)")
    print(f"QUANT_TIME_S={quant_time:.2f}", flush=True)

    model_int8.aggregator = quantized_aggregator

    # ---- STAGE 4: cuantizacion completada ----
    stage("STAGE 4: quantized aggregator assigned onto model (fp32 original still referenced)")
    ...
    # ---- STAGE 5: del de referencias FP32 + gc.collect() (THE FIX) ----
    del fp32_aggregator_ref, quantized_aggregator
    gc.collect()
    stage("STAGE 5: del(fp32_aggregator_ref, quantized_aggregator) + gc.collect() done")

    # ---- STAGE 6: estado estable INT8 ----
    ...
    stage("STAGE 6: stable INT8 state (model.eval() done)")
    ...
    images = images.to(device)
    stage(f"  loaded {images.shape[0]} real images (no dtype cast needed for weight-only quant)")
    ...
    try:
        ...
        stage("STAGE 7: int8 inference done")
        ...
        compare_outputs(kept_fp32, predictions_int8, "fp32", "int8-aggregator-corrected")
    except Exception as e:
        print(f"\nINT8 INFERENCE FAILED: {type(e).__name__}: {e}", flush=True)
        import traceback
        traceback.print_exc()

    print("\nDONE - measure_load_int8_v2 complete.", flush=True)
